...
Run sg_audit_csv_converter.py from data subdirectory against a decompressed audit log file in /sglogs. Save CSV output to sean-out.txt and any debug info in debug.txt:

```shell
python3 ./data/sg_audit_csv_converter.py /sglogs/audit.log sean-out.txt debug.txt
```

You can also open the CSV file in Excel (comma-delimited), if it doesn't have more than 64,000 rows. One could split larger log files by `S3AI` or other variable and work with many smaller files that could potentially fit into Excel. Larger CSV files could be imported to a database.

#### Sample SGAC(CSV) Output

It's a CSV file with a bunch of rows and columns.

#### Known issues and limitations

- Pyton converter script:
  - **IMPORTANT**: the script does not capture all audit messages
    - `MRSP` is one such entry. It contains a non-trivial JSON file which the script currently simply drops. You can easily find such rows with `cat audit.log | grep MRSP` and process them manually. Or better yet, fix the code and submit a pull request
